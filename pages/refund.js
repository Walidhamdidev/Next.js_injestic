export default function RefundPolicy({ shopify_email }) {
  return (
    <div className="px-20 py-20 leading-loose text-lg container mx-auto">
      ...
      <span className="font-semibold pl-1.5">{shopify_email}</span>. If your
      return is accepted, we’ll send you a return shipping label, as well as
      instructions on how and where to send your package. Items sent back to us
      without first requesting a return will not be accepted. You can always
      contact us for any return question at
      <span className="font-semibold pl-1.5">{shopify_email}</span>. Damages and
      issues Please inspect your order upon reception and contact us immediately
      if the item is defective, damaged or if you receive the wrong item, so
      that we can evaluate the issue and make it right. Exceptions /
      non-returnable items Certain types of items cannot be returned, like
      perishable goods (such as food, flowers, or plants), custom products (such
      as special orders or personalized items), and personal care goods (such as
      beauty products). We also do not accept returns for hazardous materials,
...
export function getStaticProps() {
  return {
    props: {
      shopify_email: process.env.NEXT_PUBLIC_SHOPIFY_STORE_EMAIL,
    },
  };
}
